...
#+BEGIN_SRC cpp 
  ...
  >> 
#+END_SRC
** Stdout and Stderr stream redirection 
...
Every process, has the following standard streams: 
 ...
 + *stderr* - Standard error output (file descriptor 2) => encapsulated
   ...
   + Note: Stream redirection is also possible on Windows. However, it
     is not so common as in Unix-like operating system as terminal
     application are not so used in Windows as in Unix-like platforms.
...
#+BEGIN_SRC text 
  $ ./stream-redirect.bin 125.0 
   ...
   Cubic root of a = 125 ; is approximately 5 number of iteratios is 15
#+END_SRC
...
#+END_SRC
 ...
 + Redirecto stdout - standard output stream to a file.
   + ./program > stdout.log 
