...
|-----------+-------------+------+---+----+-----+-------+-------------------------------------------|
| binary16  | -           |   16 | 1 |  5 |  10 |    15 | IEE754 half precision floating point      |
...
Note: 
  + In some platforms, the type _long double_ can be an 80-bits extended
...
Equality based on relative error in percent:

#+BEGIN_SRC cpp
  template<typename T>
  auto aeq_rel(T x, T y, T tol_pct) -> bool
  {
      constexpr auto eps = std::numeric_limits<T>::epsilon();
      // Tolerance can never be smaller than the Epsilon for this
      tol = std::max(tol, eps);
      // Missing: handle the case if X and/or Y are NaN
      return std::fabs(x - y) < tol / 100.0 * std::max(std::fabs(x), std::fabs(y)) ;
  }
#+END_SRC
...
*** Floating Point Literature and Further Reading 
