...
Notes: 
  ...
  + eps (_epsilon_) is defined as the positive number x, such that x +
    1.0 is not equal to 1.0. Two numbers X and Y, cannot be
    distinguished if |X - Y| < eps.

  + The *eps* is the maximum precision or tolerance of any numerical
    algorithm or numerical method implementation.
...
Testing float point machine precision *EPS* - epsilon. 
