...
The Windows API has several loading embedded resource, such data such
as strings, icon, menus and any arbitrary data from PE32 executables
and dynamic linked libraries DLLs. Cross platform applications should
avoid using the Windows APIs to be listed in this section, since they
are specific to Windows, therefore not portable. Applications aiming
portability can use libraries like [[https://github.com/graphitemaster/incbin/][incbin]] libraries [[https://github.com/vector-of-bool/cmrc][cmrc]] (CMake-Resource
