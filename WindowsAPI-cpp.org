...
#+END_SRC
** Loading DLLs - shared libraries at runtime - DLLs
...
#+END_SRC
 ...
 + *Load shared library* (aka module) _urlmon.dll_ at runtime using [[https://docs.microsoft.com/en-us/windows/desktop/api/libloaderapi/nf-libloaderapi-loadlibraryw][LoadLibraryW]] API
   ...
   + If the DLL is provided without absolute path as
     ~C:\Windows\System32\urlmon.dll~, the DLL is searched first at the
     current directory where is the application and then at the system
     directory - ~%SystemRoot%\system32~ or  ~C:\Windows\System32\~
...
**** Reference
   ...
   + [[https://rsdn.org/article/inet/wininet.xml][Введение в WinInet]] (In Russian.)

   + [[https://www.cnblogs.com/zhcncn/articles/3289366.html][IEWinINet API，WinInet的异步使用方法 - 金石开 - 博客园]]
