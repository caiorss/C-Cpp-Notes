...
** Simple 2D drawing with modern OpenGL 
...
subroutines applicable to VBO modify global state and assumes that
some global state is set, such as that the buffer object is bound. The
effect of calling the subroutines depends on the state and the order
...
#+BEGIN_SRC cmake 
  ...
  target_link_libraries( draw2d  glfw OpenGL::GL GLU glut)
#+END_SRC
