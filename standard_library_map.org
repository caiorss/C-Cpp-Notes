...
#+TITLE: CPP / C++ Notes - Standard Library Map 
 ...
 *Language Features* 
   ...
   * [[https://en.cppreference.com/w/cpp/language/cv][volatile specifier - cppreference]] ; [[https://docs.microsoft.com/en-us/cpp/cpp/volatile-cpp?view=vs-2019][volatile specifier - Microsft]]
     ...
     * Use cases:
       * Access memory mapped IO devices. Access hardwares mapped into
         specific memory locations assigned by manufacturers such as:
         timers, analog-to-digital-converters, digital IO and so on.
 ...
 *Fixed Size Integers* 
  ...
  + [[https://en.cppreference.com/w/cpp/header/cstdint][<cstdint>]]
    + Fixed-width integers which the byte size is
      platform-independent. Types: int8_t, uint8_t, int16_t, int32_t
      and so on.
...
WARNING: Many of those C-functions are known for introducing security
vulnerabilities such as *buffer overflow* when used without proper
care. 

Despite that the usage of those C functions is discouraged in C++,
many C code bases, low-level, legacy code and operating system
related-codes still uses them, therefore understanding them matters.
