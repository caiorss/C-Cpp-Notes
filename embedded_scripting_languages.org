 ...
 *Performance Optimization for Interpreters*
   ...
   + Compilation to a standardized virtual machine
     + Rather than implementing its own bytecode virtual machine, a
       programming language may use standardized bytecode virtual
       machine runtimes that are not exposed to external use and not
       likely to have breaking changes on every release. Some of these
       virtual machines, that are a compilation target to many
       programming languages, are: _JVM_ - Java Virtual Machine, used by
       Jython (Python for JVM), Clojure and Groovy; _CRL_ -
       Commong Language Runtime (.NET Virtual Machine), use by C#
       (CSharp), F# (FSharp), IronPython; and _WASM_ - Web Assembly.  
