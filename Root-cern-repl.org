...
#+BEGIN_SRC sh 
  $ docker run -t -i -a stdout -a stdin --entrypoint "/opt/root/bin/root.exe" mazurov/cern-root 
#+END_SRC
...
#+BEGIN_SRC sh 
  $ docker run -it --rm -w /user -v $(pwd):/user --entrypoint="/opt/root/bin/root.exe" mazurov/cern-root
#+END_SRC
