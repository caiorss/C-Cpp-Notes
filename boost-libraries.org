...
Among other thing the memory-mapped files provides the following
benefits:
 + Faster file processing, specially for big files of Gigabyte-size.
 + Persistence and serialization: Any object allocated in a
   memory-mapped file segment is automatically written to the disk.
