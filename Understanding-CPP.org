...
a particular derived class and also for checking if a pointer to base
class refers to an instance of a particular derived class.
...
Notes and remarks: 
 ...
 + _static_cast_ downcasting is unsafe, if the conversion is not
   possible, the result is undefined.
