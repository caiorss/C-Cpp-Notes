 ...
 *Advatanges*:
 ...
 + Perfomance-friendly language, C++ has many features which can help
   ...
   + Templates
   + Pointers 
   + Compiler optmizations
   + Lots of standard data structures (STL containers).
 ...
 + Processor Architecture
   ...
   + Most used for embedded systems and mobile devices.
     + *ARM core* (32 bits) - Widely used in Smartphones, PDAs, Tablets,
       ...
       and microncontrollers are produced by many manufacturers.
...
An workaround to the object slicing problem could be returning by
...
attempt to use this returned reference a destroyed object will result
...
solution to this problem is to use smart pointers, which are
stack-allocated objects that delete the pointed objects when the the
they go out of scope. It is possible to use ~unique_ptr~ which is only
...
In C++ functions pointers are not so common as in C, however many
C-libraries needed to be called from C++ APIs still use function
pointers on callbacks and event handlers. So, it is worth knowing
about function pointers in order to interface those C-APIs.

The C++ language has more powerful mechanisms for passing callbacks
such as the polymorphic type std::function which can be used to pass
function pointer, functors (function-objects) or C++11 lambdas as
arguments.
...
pointers. Instead, it is better to use the type std::function
available in the header <functional> as it can also work with 
anything callable such as C++11 lambdas, functions-objects and
function pointers.
