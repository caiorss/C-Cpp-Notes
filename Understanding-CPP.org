...
abstractions, are ubiquitous in functional programming, they were
introduced by Lisp and has made its way into mainstream and has 
arrived to C# (Cshap), Java 8 and C++11. 

Lambda expression can be passed as arguments as any function and also
be defined locally at the call-site simplifying all the boilerplate
code necessary to pass a function. In addition, they can be returned
from functions and have state. 
...
Summary:
  + Lambda functions can be:
    ...
    + Lambdas can be converted to function pointers what is useful with
      old C-APIs.
  + Lambda turns C++11 is a game changer and turns C++ into an almost
