 ...
 *C-Functions used for c-string (null-terminated chracter) array manipulation:*

Header: [[https://en.cppreference.com/w/cpp/header/cstring][<cstring>]]
 ...
 *ABI - Fragile Base Class or Fragile Binary Interface* 
 ...
 + C++ has the fragile base class problem that happens when changes in
   ...
   important for libraries or plugin-systems where third-party code is
 ...
 *Processes and Operating Systems* 
 ...
 + *Process*: A unique instance of a running program with its own PID
   (Process Identifier), address-space, virtual memory and threads. 

 + *Virtual Memory*: Portion of physical memory assigned to a process by
 ...
 + Process Isolation: One of the purposes of the virtual memory is to
   not allow a process read the memory of another process. 

 + Memory Segments: Every process, no matter the language it was
   written, has the following memory segments in the virtual memory:
 ...
 + Process State: Every process has the following states.
 ...
 + Operating System APIs - Most operating systems are written in C and
   their APIs (Application Programming Interfaces) and services are
   exposed in C language, this API can be:
   * System Calls => Documented on Linux, BSD and etc. Undocumented on Windows. 
