...
This section provides links to the C++ standard library categorized by
concept. 
 ...
 *C++17 Library Additions*
 ...
 + [[https://en.cppreference.com/w/cpp/utility/optional][std::optional]] - A container which allows returning a value or
   returning nothing. It solves problems of expressing that a function
   may return nothing in a more concise and type-safe way than usual
   approaches such as returning a null pointer, returning an empty
   object (null object pattern), throwing an exception or returning a
   tuple with boolean and a value. A common problem of the null
   pointer approach is that one may forget to check whether the value
   returned by the pointer is null. It may cause the null-dereference
   undefined behavior, which is similar to the null pointer exception
   problem that still haunts many languages like Java, C# and Python.
 ...
 *Generic Programming / Template Metaprogramming*
 ...
 + [[https://en.cppreference.com/w/cpp/language/if][if constexpr]] (C++17)
   ...
   + See:
     ...
     * [[https://www.codingame.com/playgrounds/2205/7-features-of-c17-that-will-simplify-your-code/constexpr-if][constexpr if - 7 Features of C++17 that will simplify your code]]
...
** General C++ Terminology and Concepts 
 ...
 + *Pointers*
   ...
   - _Pointer_: A variable which holds the address of another
     ...
     referencing heap-allocated objects. All C++ ordinary pointers
     (not function pointers or pointer to member functions) have the
     same size and store a numeric address of some memory location,
     the only difference between pointers of different type is the
     type of memory location that they reference.
