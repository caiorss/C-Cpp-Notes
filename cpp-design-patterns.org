...
#+BEGIN_SRC cpp 
  ...
  // Cannot infer type of of parameter T
  >> erase_equal
  ROOT_prompt_28:1:1: error: reference to overloaded function could not be resolved; did you mean to call it?
  erase_equal
  ^~~~~~~~~~~
  input_line_34:5:6: note: possible target for call
  void erase_equal(Container<T, Allocator<T>>& container, const T& value){
       ^
