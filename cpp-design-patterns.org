...
An interface class should: 
 + be on the header file, for instance, _IStack.hpp_ or any other header
   file. It does not eed any implementation .cpp file.
 + not contain any member variable
 + any concrete methods (non pure virtual member functions)
 + contain only virtual member function (abstract methods). 

Applications: 

 + _Use multiple implementations_
   + An interface class allows a client code that refers only to
     interface and no concrete implementation to use multiple
     implementations to swap and replace them at runtime.

 + _Avoid diamond problem_
   + Most programming languages avoid the diamond problem caused by
     multiple inheritance by allowing a class to inherit multiple
     interfaces and only one class or abstract class.

 + _Mock objec for testing_
   + As an interfaces allow a client code to not depend on any
     concrete implementation, the object used by the client code, for
     instance, a database object (class RealDatabase) implemeting the
     interface IDatabase which connects to a real database can be
     replaced by a mock or dummy object (class DummyDatabase) that
     simulates a database. However the dummy object is not connect to
     any real database. 

 + _Plugins systems_
   + A software can have its functionality extended at runtime by
     dynamically loading plugins. An application can load shared
     libraries (plugins) at runtime using the operating system APIs,
     LoadLibrary on Windows and dlopen on *Nix systems (Linux, MacOSX,
     BSD...). For instance, the plugins class1.dll (class Class1),
     class2.dll (class Class2) and so on, contain classes implementing
...
The Windows API - *COM (Component Object Model)* architechture.  In this
architechture the interface IUnknown allows a client code to query the
interfaces that an object implements at runtime. The architechutre
requires that all interfaces and COM - classes to implement the
interface IUnknown and the client code to communicate with classes
only using standard COM interfaces and querying interfaces at runtime.
